...
weight_status_table_low <- full_join(prop1, prop2) |>
  ...
  filter(low_wt_category  != '0.No Low Wt') |> 
  mutate(low_wt_category = recode(low_wt_category, '1.UW - No BMIZ Reduce' = '1.UW - No BMIz Reduction'))

weight_status_table_low$sex_z <- factor(weight_status_table_low$sex_z, levels = c('Girls', 'Boys'))
...
weight_status_table_high$sex_z <- factor(weight_status_table_high$sex_z, levels = c('Girls', 'Boys'))
...
theme_1 <- 
  theme_minimal() +
  theme(
  text = element_text(size = 26, family = "Avenir"),
  axis.title = element_text(size = 26, face = "bold"),
  axis.text = element_text(size = 26),
  plot.title = element_text(hjust = 0.5, size = 26, family = "Avenir", face = "bold"),
  legend.position = "top")
...
fill_colors <- c("#ffd583","#1a4e66","#fc6d46", "#c2b824")

weight_status_table_low_2 <- weight_status_table_low |> 
    mutate(`Low Wt Category` = factor(low_wt_category, levels = c("1.UW - No BMIz Reduction", "2.BMIZ Reduce - No UW", "3.UW and BMIZ Reduced"), labels = c('UW - No BMIz Reduction', "BMIZ Reduce - No UW", "UW and BMIZ Reduced")))

ggplot(data = weight_status_table_low_2,
       aes(x = sex_z, y = percent_in_sex/100, fill = `Low Wt Category`, label = percent_in_sex)) + #set the aesthetic features, including the labels at one decimal place
  ...
  theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1), text = element_text(size = 20))+ #make legend text a bit bigger
    ...
    theme_classic()+
    theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1), text = element_text(size = 20)) + #make legend text a bit bigger
   scale_fill_manual(labels = ~ stringr::str_wrap(.x, width = 15), values = fill_colors[2:4]) +
  theme_1 +
  ggtitle("Low Weight Status from Ages 13-15.99 in ALSPAC")

ggsave(file = 'uw_pct.png', width = 12, height = 12)
...
ggplot(data = weight_status_table_high,
    ...
    labs(x = element_blank(), y = 'Percent (within gender)', fontface = 'bold') +
     geom_text(aes(label = sprintf('%0.1f%%', percent_in_sex)), position = position_stack(reverse = TRUE, vjust = 0.5), color = 'white', size = 6, fontface = 'bold') +
    theme_classic()+
...
df_16$low_wt_category <- fct_collapse(df_16$low_wt_category, NoReduce = c("1.UW - No BMIZ Reduce", "0.No Low Wt"))

df_16 <- within(df_16, low_wt_category <- relevel(low_wt_category, ref = 'NoReduce'))
...
binge_results_f <- glm(binge_14_16 ~ high_wt_category, data = df_16_f, family = 'binomial')
binge_results_m <- glm(binge_14_16~ high_wt_category, data = df_16_m, family = 'binomial')
...
compensatory_table <- frq_table_by_wt_sex(df_16, reg_compensatory) |>
  ...
  mutate(sex_z = recode(sex_z, '1' = 'Boys', '2' = 'Girls')) |>  
  mutate(`Low Wt Category` = factor(low_wt_category, levels = c("0.No Low Wt", "1.UW - No BMIZ Reduce", "2.BMIZ Reduced - No UW", "3.UW and BMIZ Reduced"), labels = c("No Low Wt", 'UW - No BMIz Reduction', "BMIZ Reduced - No UW", "UW and BMIZ Reduced")
                                    )
  )

ggplot(data = compensatory_table,
    aes(x = `Low Wt Category`, y = pct, fill = low_wt_category, label = sprintf('%0.1f%%', pct*100))) + #set the aesthetic features, including the labels at one decimal place
    geom_bar(position = position_stack(reverse = TRUE), stat = 'identity') + #add column graph, with male and female separated
    scale_y_continuous(labels = scales::percent, limits = c(0,0.22))+ #set the lower and upper limits of the y axis ( 0-100 percent);
    theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1), text = element_text(size = 15))+ #make legend text a bit bigger
    labs(x = element_blank(), y = 'Percent') +
     geom_text(aes(label = sprintf('%0.1f%%', pct*100)), position = position_stack(reverse = TRUE, vjust = 0.5), color = 'white', size = 4, fontface = 'bold') +
    facet_wrap(~sex_z) +
    theme_minimal()+
    labs(title = "Weight Control Behavior Endorsement at Age 16 \n by Low Weight Group (13-15.99)") +
    theme_1+
    theme(legend.position = 'none',
          axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 12),
          plot.title = element_text(size = 18)) +
       scale_fill_manual(values = c("#ffd583","#fc6d46",  "#1a4e66", "#c2b824"))

ggsave(file = 'compensatory_table.png')

```

```{r}
#| label: fig-binge16
#| fig-cap: "Age 13 weight status vs. Age 16 BEr Engagement"

binge_table <- frq_table_by_wt_sex_high(df_16, binge_14_16) |>
  filter(binge_14_16 == 1) |>
  mutate (sex_z = recode(sex_z, '1' = 'Boys', '2' = 'Girls'))
...
ggplot(data = binge_table,
    ...
    geom_bar(position = position_stack(reverse = TRUE), stat = 'identity') + #add column graph, with male and female separated
    scale_y_continuous(labels = scales::percent, limits = c(0,0.16))+ #set the lower and upper limits of the y axis ( 0-100 percent);
    theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1), text = element_text(size = 15))+ #make legend text a bit bigger
    labs(x = element_blank(), y = 'Percent') +
     geom_text(aes(label = sprintf('%0.1f%%', pct*100)), position = position_stack(reverse = TRUE, vjust = 0.5), color = 'white', size = 4, fontface = 'bold') +
    facet_wrap(~sex_z) +
...
df_14 <- within(df_14, low_wt_category <- relevel(low_wt_category, ref = '1.UW- No BMIZ Reduce'))
df_14 <- within(df_14, high_wt_category <- relevel(high_wt_category, ref'1.UW- No BMIZ Reduce'))
